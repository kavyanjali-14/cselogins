...
let students = JSON.parse(fs.readFileSync("./CseCrud/data/students.json"));
...
app.post("/cse/post", (req,res) => {
  ...
  fs.writeFile("./CseCrud/data/students.json", JSON.stringify(students), (err) => {
...
app.patch("/cse/updatebyid/:id",(req,res) => {
  let id = req.params.id;
  let stuupdate = students.find(el => el.id == id);
  if(!stuupdate)
  {
    res.send("Not Data found with id");
  }
